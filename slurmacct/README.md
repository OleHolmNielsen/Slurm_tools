...
The ```jobacctstats``` tool prints information for all jobs using Slurm sacct accounting records.
...
```
Usage: jobacctstats [-s Start_time -e End_time | -c | -w | -m monthyear] [-p partition(s)] [-r report-prefix] [-n] [-h]
where:
	-s Start_time [last month]: Starting time of accounting period.
	-e End_time [last month]: End time of accounting period.
	-c: Current month
