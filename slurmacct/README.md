...
Note: Since the ```topreports``` tool prints Slurm parent accounts using  ```slurmacct -P```,
this option requires the Slurm accounts PARENTFILE ```/etc/slurm/accounts.conf```.
The PARENTFILE can be conveniently generated by the ```slurmaccounts2conf``` tool
in [Slurm accounts and users](../slurmaccounts/) which dumps the Slurm associations
